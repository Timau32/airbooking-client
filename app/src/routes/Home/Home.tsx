...
const Home = () => {
  const [homeData, setHomeData] = useState<{ popular: any[]; holidays: any[]; regions: any[] }>({
    popular: [],
    holidays: [],
    regions: [],
  });
  ...
  const fetchHomeData = async () => {
    try {
      ...
      const contries = await api.getContries();
      const regions = (await api.getRegions(contries.data.results[0].slug)).data.results;

      console.log({ holidays, contries, regions });
      setHomeData({ popular: lattestMocks, holidays, regions });
    } catch (err) {
      console.log(err);
      message.error(
        ...
        <Container>
          ...
          <div className={classes.latest_apartments}>
            <Flex className={classes.cities}>
              {homeData.regions.map((regions, idx) => (
                <div className={classes.latest_item} key={`regions-${regions.id}`}>
                  <div className={classes.latest_body}>
                    ...
                    <img src={citiesMocks[idx].images} alt={regions.name} />
                    <Typography.Paragraph className={classes.latest_title}>{regions.name}</Typography.Paragraph>
                  </div>
                </div>
              ))}
            </Flex>
